...
- **Phase C NVS versioning + port-field removal (2026-09-20)**: audit items #12/#13/#16 in `main.c` — `node_cfg_t` gained `uint8_t version` (`CFG_VERSION 2`); `cfg_load` now requires `len == sizeof(node_cfg)` AND `version == CFG_VERSION` (IDF v6.1 `nvs_api.cpp` copies a shorter blob and returns OK, so ESP_OK alone proves nothing — stale tail), logging `want v2 len 115, got v77 len 114` on mismatch and zeroing the struct; `server_port` deleted from struct, form and handler (stored/displayed but consumed by nothing — the board always listens on `UDP_PORT` 1234). **Hardware gate on the flashed image**: old v1 blob rejected (`v77` = `0x4D` = first byte of the old SSID — the silent-garbage failure mode #12 guards, demonstrated live) → `no usable config` → setup AP clean, no panic. **Consequence**: the node needs ONE re-provision from any Wi-Fi client (which also closes #23 and the Phase B wire gate; the v2 save→load round trip verifies then). Log: `logs/2026-09-20_phaseC-nvs.md`
...
| **P16: Phase C — NVS config versioning** | ✅ Rejection path VERIFIED on HW 2026-09-20; v2 save→load round trip pending the one-time re-provision | `main/main.c` | `node_cfg_t` + `uint8_t version` (`CFG_VERSION 2`); `cfg_load` requires exact len AND version (shorter-blob-OK trap documented); `server_port` removed from struct/form/handler. Old v1 blob rejected live (`got v77 len 114`), setup AP clean, no panic. Log: `logs/2026-09-20_phaseC-nvs.md` |
...
13. 🔲 Portal + AP-join hardware round trip (one Wi-Fi client, ~2 min): join `AudioNode-Setup` → #23 join/leave lines print; POST `/save` with `server_ip=999.1.1.1` → HTTP 400 "not a valid IPv4 … nothing was saved"; POST valid config → `cfg: saved via portal` → reboot → STA → GOT IP; next boot loads the v2 blob (Phase C round trip) and streaming resumes
...
One phone session on `AudioNode-Setup` closes all three pending wire items at once:
(#23) join → `ap: station ... joined/left` lines print; (B) POST `/save` with a bad IP →
HTTP 400 + nothing saved, then valid → save → reboot → STA; (C) the re-provision itself —
the version gate rejects the old v1 blob by design, so the node needs it anyway; the v2
save→load round trip and streaming resume verify in the same two minutes. Until then the
node is intentionally in setup-AP mode (config rejected, not lost — the NVS blob is intact).
