...
13. 🔲 Portal + AP-join hardware round trip (one Wi-Fi client, ~2 min): join `AudioNode-Setup` → #23 join/leave lines print; POST `/save` with `server_ip=999.1.1.1` → HTTP 400 "not a valid IPv4 … nothing was saved"; POST valid config → `cfg: saved via portal` → reboot → STA → GOT IP
14. 🔲 Commit each verified milestone only

**Audit backlog (from `docs/AUDIT.md` — 10 valid items remaining, phased):** next firmware
work is **Phase C** (NVS config version field + blob-length check — must land before the
#16 `node_cfg_t` change, which rides with it), then **D** (silence-fill cap, `tone_ms`,
...
Phase A (diagnostics) ✅ done as P14; Phase B (portal hardening) ✅ done as P15. Two open
hardware items need only a Wi-Fi client near the board: (#23) join `AudioNode-Setup` once —
the `ap: station ... joined/left` lines print themselves; (B wire gate) submit the portal
form once with a bad IP (expect 400 + nothing saved) then a valid one (expect save →
reboot → STA).
