...
- **Phase B portal hardening (2026-09-19)**: audit items #15/#14/#27/#11 implemented in `main.c` — (15) `server_ip` validated by a strict dotted-quad checker *before any NVS write*, HTTP 400 + "nothing was saved" on failure (the old path saved garbage, then boot silently degraded the whitelist to accept-any); (14) `strlen` hoisted out of the `url_decode` loop (O(n²)→O(n)); (27) `const char *html`; (11) idiomatic `strtok_r`. The validator is deliberately stricter than lwIP `ip4addr_aton` (which accepts `0x`/octal/`a.b.c` forms per `ip4_addr.c:165-193`) and rejects `0NN` octets so a saved string always parses back as plain decimal. `#16 server_port` re-slotted to Phase C. Logic gate `tmp/verify_phaseB.py` 21/21; flashed + 40 s stream at exactly 50.0 pps `dropped=0` byte-exact, heartbeat intact, no panic; portal wire round-trip pending HW (no second netif — see E-13). Log: `logs/2026-09-19_phaseB-portal.md`
- **Phase C NVS versioning + port-field removal (2026-09-20)**: audit items #12/#13/#16 in `main.c` — `node_cfg_t` gained `uint8_t version` (`CFG_VERSION 2`); `cfg_load` now requires `len == sizeof(node_cfg)` AND `version == CFG_VERSION` (IDF v6.1 `nvs_api.cpp` copies a shorter blob and returns OK, so ESP_OK alone proves nothing — stale tail), logging `want v2 len 115, got v77 len 114` on mismatch and zeroing the struct; `server_port` deleted from struct, form and handler (stored/displayed but consumed by nothing — the board always listens on `UDP_PORT` 1234). **Hardware gate on the flashed image**: old v1 blob rejected (`v77` = `0x4D` = first byte of the old SSID — the silent-garbage failure mode #12 guards, demonstrated live) → `no usable config` → setup AP clean, no panic. **Consequence**: the node needs ONE re-provision from any Wi-Fi client (which also closes #23 and the Phase B wire gate; the v2 save→load round trip verifies then). **Re-provisioned 2026-09-20 evening from this PC's own Wi-Fi** (`AudioNode-Setup` → portal → save → `server whitelist <pc-ip>` + `STA: joining` + GOT IP, no rejection — v2 round trip closed); streaming byte-exact at 50.0 fps (2-6% air loss today, RF environment — rssi dips to -63; identical RX path ran 0% in the Phase B session). Log: `logs/2026-09-20_phaseC-nvs.md`
- **Phase D stream/timer/handle hygiene (2026-09-20)**: audit #17/#10/#6/#28 in `main.c` (P17) — silence fill capped by `ring_free()/FRAME_BYTES` (mutex-held read) so a 64-frame worst-case gap can never crowd real PCM out of the ring; `tone_ms` to task scope + stale `tcp_task` comment fixed; failover timer resets in the `ap_active` branch (inert today, E-2); static `httpd_handle_t` + `httpd_stop()` guard. **#18 rejected**: `int64_t last_pkt_ms` would introduce a torn-read race (Xtensa 64-bit access = two halves); the `uint32_t` wrap pattern is atomic and correct (E-15). HW gates on the flashed image: 60 s tone → 3000 sent / 2984 accepted / `total=5729280` byte-exact at 50.0 pps, ring 44–47 KB (never saturated), 0.5% air loss; mid-stream kill (sender dead at 650/13.0 s) → restart → board re-accepted at exactly 50 pps (`pkts=944→1194 dropped=3`, ring 8–12 KB), sender2 750/15.0 s, zero panic/watchdog markers. Failover re-verify deferred (portal cycle needs the user-held Wi-Fi password; #6 inert today, path verified 2026-09-14 as P6). Log: `logs/2026-09-20_phaseD-hygiene.md`
...
| **P15: Phase B — portal input hardening** | ✅ Logic gate 21/21 + flash/stream VERIFIED 2026-09-19; portal wire round-trip pending HW | `main/main.c` | #15 strict IPv4 validation before any NVS write (400 + nothing saved), #14 `strlen` hoist, #27 `const html`, #11 idiomatic `strtok_r`; #16 re-slotted to Phase C. 40 s stream exactly 50.0 pps `dropped=0` byte-exact. Log: `logs/2026-09-19_phaseB-portal.md` |
| **P16: Phase C — NVS config versioning** | ✅ VERIFIED 2026-09-20 — rejection path live (`got v77 len 114`) AND v2 round trip closed by re-provision | `main/main.c` | `node_cfg_t` + `uint8_t version` (`CFG_VERSION 2`); `cfg_load` requires exact len AND version (shorter-blob-OK trap documented); `server_port` removed from struct/form/handler. Post-provision boot: `server whitelist` + `STA: joining` + GOT IP, no rejection; streaming byte-exact (2-6% air loss today, RF — see log). Log: `logs/2026-09-20_phaseC-nvs.md` |
| **P17: Phase D — stream/timer/handle hygiene** | ✅ VERIFIED on HW 2026-09-20 (60 s + kill/restart gates); failover re-verify deferred (needs portal cycle with the Wi-Fi password) | `main/main.c` | #17 silence-fill capped by `ring_free()/FRAME_BYTES` (mutex-held); #10 `tone_ms` task-scope init; #6 failover `sta_start` reset; #28 static `hd` + `httpd_stop` guard; **#18 rejected** — `int64_t` would introduce a torn-read race where the `uint32_t` pattern is atomic + wrap-correct (E-15). 60 s: 2984 accepted, byte-exact, ring 44–47 KB; mid-stream kill → restart re-accepted at exactly 50 pps; zero panic markers. Log: `logs/2026-09-20_phaseD-hygiene.md` |
...
13. 🔲 Next setup-AP session (do NOT factory-reset just for this): capture #23 `ap: station joined/left` lines and the Phase B bad-IP → HTTP 400 wire test; the PC's Wi-Fi adapter can be the portal client now
14. 🔲 Commit each verified milestone only

**Audit backlog (from `docs/AUDIT.md` — 3 valid items remaining, phased):** next firmware
work is **Phase E** (#32 drive the amp's SD pin HIGH only after `i2s_init()` — 1-line
reorder; #29 `esp_wifi_stop()` before `nvs_flash_erase()` in the factory-reset task), then
**F** (#8 document the Xtensa 32-bit-atomicity assumption on the flags; #20 lock-free ring
only against a measurement; #25 `PRIu32` cosmetic). Phases A–D ✅ done as P14–P17 (#18
rejected, E-15). Node online, v2 config, streaming byte-exact. Open wire items wait for the
next natural setup-AP session (do NOT factory-reset just for them): (#23) capture
`ap: station joined/left` lines, (B) bad-IP → HTTP 400 test — the PC's Wi-Fi adapter can be
the portal client. Optional assisted demo: live failover re-verify (provision a throwaway
SSID → 30 s → setup AP → re-provision) needs the Wi-Fi password through the portal.
