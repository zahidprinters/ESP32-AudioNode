...
**Audit backlog (from `docs/AUDIT.md` — 8 valid items remaining, phased):** next firmware
work is **Phase D** (silence-fill cap, `tone_ms` init, failover timer reset, `httpd_stop`
guard, `int64_t` ms), then **E** (amp enable after `i2s_init`, `esp_wifi_stop` before NVS
erase), **F** (polish, only against a measurement). Phases A/B/C ✅ done as P14/P15/P16.
Node is back online: re-provisioned 2026-09-20 from this PC's own Wi-Fi (`AudioNode-Setup`
→ portal → save → STA → GOT IP), v2 blob loads clean, streaming byte-exact. Remaining wire
items wait for the next natural setup-AP session (do NOT factory-reset just for them):
(#23) capture `ap: station joined/left` lines, (B) bad-IP → HTTP 400 test. Note: the PC's
Wi-Fi adapter can now serve as the portal client (it took a 192.168.4.2 lease during
provisioning). Today's air loss ran 2-6% (rssi dips to -63) — environmental; if audible,
options: move the board closer to the AP, or investigate in a later session.
